...
from custom_models import NeuMF, BPR, UserKNN, ItemKNN, FM, SpectralCF, DeepFM, EASE, NFM, DCNV2
...
class AmazonGroceryFeedbackLoop():
    def __init__(self, config: DotDict, initialization_dataset: AmazonBooksDataset, **kwargs):
        ...
        self.cold_start_unrolled_dataset_total_path = f"./cache/cold_start_unrolled_total_{self.initialization_dataset.y1}-{self.initialization_dataset.y2}_amazon_grocery.csv"
        self.user_rating_probs_path = f"./cache/user_rating_probs_{self.initialization_dataset.y1}-{self.initialization_dataset.y2}_amazon_grocery.csv"
        self.metric_dict = {
            "epoch_index": 0,
        }
    ...
    def init_experiment(self) -> None:
        if not self.config.use_cache or not os.path.exists(self.cold_start_unrolled_dataset_total_path):
            ...
            #     self.dataset_unrolled_cold_start["item_id"].isin(active_items)
            # ]

            print(f"\n Users count: {len(self.dataset_unrolled_cold_start['user_id'].unique())} \n Items count: {len(self.dataset_unrolled_cold_start['item_id'].unique())} \n")
            self.dataset_unrolled_cold_start.to_csv(self.cold_start_unrolled_dataset_total_path)
        else:
            self.dataset_unrolled_cold_start = pd.read_csv(self.cold_start_unrolled_dataset_total_path, index_col=0)
            self.cold_start_start_date = pd.to_datetime(self.dataset_unrolled_cold_start['date']).min().date()
            self.cold_start_end_date = self.cold_start_start_date + relativedelta(months=self.config.cold_start_months) - relativedelta(days=1)
            self.start_experiment_date = self.cold_start_end_date + timedelta(days=1)
    ...
    def _build_window_for_epoch(self, k: int = 0, new_interactions = None):
        ...
        if k == 0:
            ...
            item_feats = (
                ...
                .loc[:, ["item_id", "price", "category_seq", "avg_rating"]]
            )
    ...
    def tuning_hyperparameters(self) -> None:
        hyper_file = f"tuning_parameters_amazon_grocery/{self.model_name_config}.hyper"
        export_result_file = f"tuning_parameters_amazon_grocery/{self.model_name_config}.result"
